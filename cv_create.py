...
SECTIONS = [
    ...
    ('工作/实习经历', [
        ...
        ('bullet', '智能 FA 系统（2026.03–至今｜后端开发）', '① 独立完成 AI 看板（大模型对话式数据分析）：Java+Python 分工、异步队列+多轮会话，LLM 直接产出 ECharts 图表（存 S3），9 个 REST 接口，504 超时治理。② 40+ 业务 REST 接口开发与优化：断带智能监测（判重/环比 Top/判责导出）、FTA 因子推理（失效特征回溯根因、部门级数据权限）、风险监测、改善库、MU 产品线、桌面云外网示警等；持续 SQL 调优、导出异步化。③ 量产改善定时任务优化：新增第三步“根因推荐（原因+改善措施）暂存”，责任人确认后正式落库，单条失败不阻断整批'),
