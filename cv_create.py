...
SECTIONS = [
    ...
    ('工作/实习经历', [
        ('entry', '2025.12-至今', '宁德时代新能源科技股份有限公司', '工艺算法工程师'),
        ('bullet', '质量智能体（2026.06–至今｜Agent 开发）', '开发 Neo4j FTA 知识图谱读写（670 节点）、Milvus 向量检索（3000+ chunks、768 维）等 MCP 工具与相关性分析矩阵；构建结果展示闭环（自动落库、权限共享）'),
        ('bullet', '智能 FA 系统（2026.03–至今｜后端开发）', '独立完成 AI 看板（大模型对话式数据分析）：Java+Python 分工、异步队列+多轮会话，LLM 直接产出 ECharts 图表（存 S3），9 个 REST 接口，504 超时治理'),
        ('bullet', '30+ 业务 REST 接口开发与优化', '断带智能监测（判重/环比 Top/判责导出）、风险监测、改善库、MU 产品线、桌面云外网示警等；持续 SQL 调优、导出异步化'),
        ('bullet', '量产改善定时任务优化', '新增第三步“根因推荐（原因+改善措施）暂存”，责任人确认后正式落库，单条失败不阻断整批'),
        ('bullet', '智能工艺设计（2026.03-2026.05｜后端开发）', '工艺文档向量化管理（Milvus/HNSW、文件级查重、启停）'),
        ('bullet', '厂房用电能耗项目（2025.12-2026.03）', '负责 Hive/Iceberg/Doris 数据治理（统一指标口径/数据质量）、构建 SQL+Airflow 数据流；参与 Python 数据平台产品化'),
        ('bullet', 'AI 提效', '熟练运用自研 OpenCode（接入 DeepSeek/GLM/Qwen/MiniMax 等）与 GitHub Copilot 辅助 SQL/Python/Java 开发'),
        ('entry', '2025.10-2025.12', '宁德时代新能源科技股份有限公司', '研发实习生（厂房用电能耗项目）'),
        ('bullet', '', '参与项目需求调研与设备编码、工序量测等多源数据梳理'),
    ...
    ('科研经历', [
        ...
        ('entry', '2023-2024', '两篇独立作者（唯一作者）国际会议论文', 'EI/CPCI/Scopus 收录'),
        ('bullet', '', '① AETS 2024（米兰）：前后轴独立驱动车辆扭矩分配优化（PID+PSO）；② CONF-CIAP 2023：Brief Analysis of Aeroengine（航空发动机分析）'),
